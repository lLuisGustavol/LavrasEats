...
import type { Restaurante, Avaliacao, Usuario } from "../types"
import { restauranteService, avaliacaoService, usuarioService } from "../services/api"
import { useAuth } from "../contexts/AuthContext"
import { categorias } from "../utils/categorias"


const RestauranteDetalhes: React.FC = () => {
  ...
  const carregarRestaurante = async () => {
    try {
      const response = await restauranteService.consultar(Number(id))
  ...
  const carregarAvaliacoes = async () => {
    try {
      ...
      const avaliacoesRestaurante = response.data.filter((a: Avaliacao) => {
        // Verifica se o restaurante é um objeto ou um ID
        if (typeof a.restaurante === 'object' && a.restaurante !== null) {
          return a.restaurante.id === Number(id)
        } else {
          return a.restaurante === Number(id)
        }
      })
      ...
      const avaliacoesComUsuario = avaliacoesRestaurante.map((avaliacao: Avaliacao) => {
  ...
  const confirmarDeletarRestaurante = async () => {
    try {
      await restauranteService.deletar(Number(id))
      fecharModalConfirmacao()

      setMensagemToast("Restaurante deletado com sucesso!")  // mostra toast
      ...
      setMensagemToast("Erro ao deletar o restaurante.")
      fecharModalConfirmacao()
    }
  }
  ...
  const abrirModalEdicao = () => {
    if (restaurante) {
      ...
      const categoriaEncontrada = categorias.find((g) => g.label === restaurante.categoria)
      const categoriaValor = categoriaEncontrada?.valor || ""

      setEditData({ ...restaurante, categoria: categoriaValor})
  ...
  // Handler para atualizar campos do restaurante
  ...
  const salvarEdicao = async () => {
    ...
    try {
      ...
      await restauranteService.atualizar(Number(editData.id), dadosParaSalvar)
      setModalEdicaoAberto(false)
      await carregarRestaurante()
      setMensagemToast("Restaurante atualizado com sucesso!")
    } catch (error: any) {
      setEditErro(error.response?.data?.erro || "Erro ao atualizar restaurante. Tente novamente.")
    } finally {
  ...
  if (!restaurante) {
    return (
      <div className="min-h-screen bg-[#09090B] flex items-center justify-center">
        <div className="text-white text-xl">Restaurante não encontrado</div>
      </div>
    )
  }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="card-dark flex flex-col lg:flex-row gap-8 items-start lg:items-stretch mb-10">
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold mb-2 text-white drop-shadow-lg">{restaurante.nome}</h1>
              ...
              <div className="mb-4">
                ...
                <p className="text-gray-300 leading-relaxed mt-1">{restaurante.descricao}</p>
              </div>
            </div>
        ...
        {mensagemIA && (
          <div className="card-dark mb-10">
            ...
            <p className="text-gray-400 text-sm mb-4">Essa é a análise da sua avaliação sobre este restaurante.</p>
        ...
        {isAuthenticated && (
          <div className="card-dark mb-10">
            <h2 className="subtitle mb-4">Faça sua avaliação sobre o restaurante</h2>
            {avaliacaoUsuario ? (
      ...
      {modalAberto && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
          <div className="bg-[#0F0F11] rounded-lg p-6 max-w-sm w-full text-white">
            ...
            <p className="mb-6">Tem certeza que deseja deletar este restaurante?</p>
            <div className="flex justify-end space-x-4">
              ...
              <button
                onClick={confirmarDeletarRestaurante}
      ...
      {modalEdicaoAberto && editData && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
          <div className="bg-[#0F0F11] rounded-lg p-8 max-w-lg w-full text-white">
            <h3 className="text-2xl font-bold mb-6">Editar Restaurante</h3>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="endereco" value={editData.endereco} onChange={handleEditChange} className="input-dark" />
                </div>
                <div>
              ...
              <div>
                ...
                <input name="telefone" value={editData.telefone} onChange={handleEditChange} className="input-dark" />
              </div>
              <div>
