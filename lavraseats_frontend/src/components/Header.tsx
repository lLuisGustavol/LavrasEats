...
import { Search, User, LogOut, Filter, Settings, ChevronDown } from "lucide-react"
import { useAuth } from "../contexts/AuthContext"
import { categorias as categoriasPFiltro } from "../utils/categorias"
...
const Header: React.FC = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchTerm.trim()) {
      ...
      // Se só tem filtro de categoria, navega para busca com filtro
      navigate(`/buscar?categoria=${categoriaFiltro}`)
    }
  }
  ...
  const handleCategoriaChange = (categoria: string) => {
    ...
    if (categoria) {
      // Navega diretamente para busca com filtro de categoria
      navigate(`/buscar?categoria=${categoria}`)
    } else {
      // Se selecionou "Todas as categorias", navega para busca sem filtro
      navigate(`/buscar`)
    }
  }
  ...
  const categoriaSelecionada = categorias.find(c => c.valor === categoriaFiltro)?.label || "Filtrar"
          ...
          <div className="space-y-2 overflow-y-auto flex-1">
            {categorias.map((categoria) => (
              <button
                key={categoria.valor}
                type="button"
                onClick={() => handleCategoriaChange(categoria.valor)}
                className={`w-full text-left px-4 py-3 rounded-lg hover:bg-[#23232a] transition-colors ${categoria.valor === categoriaFiltro ? 'bg-gray-600 text-white' : 'text-gray-300'
                  }`}
              >
                {categoria.label}
              </button>
      ...
      <div className="container mx-auto flex items-center justify-between">
        <div className="flex items-center space-x-8">
          <Link to="/home" className="flex items-center space-x-2">
            <img 
              src="/imgs/pinguim.png" 
              alt="Pinguim Lavras Eats" 
          ...
          <nav className="hidden md:flex space-x-6">
            ...
            <Link
                to="/recomendacoes"
                className={`transition-colors ${location.pathname === '/recomendacoes'
                ? 'text-white border-b-2 border-white pb-1'
                : 'text-gray-300 hover:text-white hover:border-b-2 hover:border-white hover:pb-1'
                }`}
            >
              ...
              <>
                ...
                {usuario?.cargo === "gerente" && (
                  <Link
                    ...
                    className={`transition-colors ${location.pathname === '/cadastro-restaurante'
                      ? 'text-white border-b-2 border-white pb-1'
                      : 'text-gray-300 hover:text-white hover:border-b-2 hover:border-white hover:pb-1'
                      }`}
                  >
        ...
        <div className="flex items-center space-x-4">
          <form onSubmit={handleSearch} className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`px-4 py-3 rounded-lg border transition-colors duration-200 flex items-center justify-center ${categoriaFiltro
              ...
              title={categoriaFiltro ? `Filtro ativo: ${categoriaSelecionada}` : "Filtrar por categoria"}
            >
